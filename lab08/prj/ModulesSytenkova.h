#ifndef MODULESSYTENKOVA_H_INCLUDED
#define MODULESSYTENKOVA_H_INCLUDED

double s_calculation ( double x, double z);
float sale (float buy);
void whiteness (int siz);
int to_binary (int N);
void task_ten_one (std::string input_file, std::string output_file);
void task_ten_two(const std::string& input_file);
void task_ten_three(int x, int z, int b, const std::string& output_file);

#endif // MODULESSYTENKOVA_H_INCLUDED
